...
def ReadData(filesLocation, filesList, numberOfFiles, WND_HEIGHT, WND_WIDTH, WND_SHIFT, VEC_PER_WND, transDir=''):
    ...
    with open(filesList) as listHandler:
        imgNbr = 0
        # Đọc danh sách file và dọn dẹp khoảng trắng dư thừa
        imageFiles = [line.strip() for line in listHandler.readlines() if line.strip()][0:numberOfFiles]

        for imageFile in imageFiles:
            # tfile sẽ là đường dẫn tương đối (vd: 000033/tên_ảnh)
            tfile = imageFile

            ################################################################
            # 1. Nạp nhãn (Transcriptions)
            found_target = False
            if transDir != '':
                # Dùng os.path.join để an toàn trên Windows
                targetFile = os.path.join(transDir, tfile + cfg.LabelFileType)
                    ...
                    with open(targetFile) as f:
                        ...
                        if len(data) == 0:
                            ...
                            for i in range(len(data)):
                                # Loại bỏ ký tự lạ ở đầu/cuối dòng trước khi chuyển sang số
                                line_content = data[i].strip()
                                if line_content:
                                    targetData = np.fromstring(line_content, dtype=np.uint16, sep=' ')
                                    targetList.append(targetData)
                        found_target = True
                else:
                    print(f"⚠️ Thiếu nhãn: {targetFile} -> Bỏ qua mẫu này.")
                    continue # Nếu không có nhãn thì bỏ qua luôn, không đọc ảnh nữa

            ################################################################
            # 2. Xử lý ảnh (Images)
            imageFilePath = os.path.join(filesLocation, tfile + cfg.ImageFileType)
            
            if not os.path.exists(imageFilePath):
                print(f"⚠️ Thiếu ảnh: {imageFilePath}")
                if found_target: targetList.pop() # Xóa nhãn tương ứng nếu không thấy ảnh
                continue

            # Đọc ảnh xám
            image = cv2.imread(imageFilePath, cv2.IMREAD_GRAYSCALE)
            if image is None:
                print(f"❌ Không thể mở file ảnh: {imageFilePath}")
                if found_target: targetList.pop()
                continue

            h, w = np.shape(image)

            # Resize giữ nguyên tỷ lệ theo chiều cao cửa sổ
            ...
            # Tính toán độ dài chuỗi (Sequence Length)
            winId = 0
            wpd = 0
            while True:
                s = (winId * WND_SHIFT)
            ...
            ################################################################
            # 3. Cắt đặc trưng (Sliding Window)
            featuresSet = []
            winId = 0
            while True:
                s = (winId * WND_SHIFT)
                ...
                if e > w:
                    # Padding cho phần cuối của ảnh nếu không đủ chiều rộng cửa sổ
                    ...
                    # Thêm padding cho các chuỗi ngắn
                    pad_full = np.ones((WND_HEIGHT, WND_WIDTH), np.uint8)*255
                    for i in range(wpd): featuresSet.append(pad_full)
                    break
    ...
    if transDir != '':
        assert len(inputList) == len(targetList), f"Lỗi: Số lượng ảnh ({len(inputList)}) và nhãn ({len(targetList)}) không khớp!"

    print(f"🚀 Load xong: {imgNbr} mẫu dữ liệu.")
